...
因此安装过程中的“安装 Python 依赖”默认不需要联网。新版安装器会强制优先使用这个目录里的离线依赖包；如果这个目录缺失或文件不全，安装器会直接报错，不会偷偷联网下载。
...
- 创建数据库。
- 创建 `.env` 配置文件。
- 安装 Python 依赖，默认只从 `installer\wheels` 离线安装。
- 初始化数据库表。
- 收集静态文件。
- 创建 ERP 管理员。
- 执行生产预检。
...
如果安装过程中看到 Python 依赖正在联网下载，通常说明使用的是旧安装包，或 `ERP安装包\installer\wheels` 没有完整拷贝。请换用最新 `ERP安装包.zip`，并确认 `installer\wheels` 下能看到 `django-6.0.6-py3-none-any.whl`、`psycopg_binary-3.3.4-cp312-cp312-win_amd64.whl` 等文件。
